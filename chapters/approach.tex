...
\begin{algorithm}
	\DontPrintSemicolon
	\SetAlgoLined
	\KwIn{$node3$: a depth 3 node, $depth$: current node depth, $key$: array of key parts,\\ $processed$: state of processed keyparts, $processed\_nodes$: dictionary of processed nodes}
	...
	\Begin{
		\ForEach{$key\_pos$ in $\{e | e \in$ dom($processed$)$, processed[e] = 0\}$}{
			...
			$next\_processed \gets processed$ \;
			$next\_processed[key\_pos] =1$ \;
